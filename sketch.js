...
function setup() {
  ...
  info.html("Click on the 'RESET' button before playing. This is a 4 player game. So if you are playing on the same device, then duplicate the tab 4 times to play.")
  info.position(window.innerWidth/30, window.innerHeight/1.1);
...
function draw() {
  ...
  if(gs === "result" && ansAr.length===pc && nameAr.length===pc && nameAr.includes(undefined)===false) {
    ...
    ANSWER = createElement("h2").html("Correct Answer is- 2 : ENVELOPE").position(window.innerWidth/1.7, window.innerHeight/1.35).style("color", "lime").style("backgroundColor", "black")
